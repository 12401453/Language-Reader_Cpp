...
#define POST 4
    ...
    else if (get_or_post == GET)
    {
        ...
        char msg_url[4096];
        memset(msg_url, '\0', 4096);
        std::cout << "size of msg_url array: " << strlen(msg_url) + 1 << std::endl;
      ...
      //#include docRoot.cpp
        ...
        int url_size = strlen("HTML_DOCS") + strlen(msg_url) + 1; //sizeof() can be used for c-strings declared as an array of char's but strlen() must be used for char *pointers
        //char url_c_str[url_size];
        char url_c_str[4096];
        memset(url_c_str, '\0', 4096);

        strcpy(url_c_str, "HTML_DOCS");
        strcat(url_c_str, msg_url);

        bool cookies_present = false;
        if(page_type > 0) cookies_present = readCookie(m_cookies, msg);
...
int WebServer::c_strFindNearest(const char* haystack, const char* needle1, const char* needle2) {
    ...
    char needles_buf[4096];
    memset(needles_buf, '\0', 4096);
...
void WebServer::buildPOSTedData(const char* msg, bool headers_present, int length) {
    ...
    if (headers_present) {
        ...
        char content_length_str[4096];
        memset(content_length_str, '\0', 4096);
...
void WebServer::handlePOSTedData(const char* post_data, SOCKET clientSocket) {
    ...
    int equals_positions_arr[512];
    memset(equals_positions_arr, '\0', 512);
    int equals_pos = 0;
    ...
    int amp_positions_arr[512];
    memset(amp_positions_arr, '\0', 512);
    int amp_pos = 0;
    ...
    for (int i = 0; i < post_fields; i++) {
        ...
        if (ith_post_value_length > 8192) {
            ...
            char ith_post_value[8192];
            memset(ith_post_value, '\0', 8192);
            std::cout << "length of post_value array: " << ith_post_value_length << std::endl;
...
bool WebServer::addText(std::string _POST[3], SOCKET clientSocket) {
    ...
    if (!sqlite3_open(m_DB_path, &DB)) {
       ...
       /* icu::UnicodeString weird_comma;
        ...
        /*unicode_text_title.findAndReplace(weird_comma, "\'"); */
...
bool WebServer::retrieveText(std::string text_id[1], SOCKET clientSocket) {
...
bool WebServer::retrieveTextSplitup(std::string _POST[3], SOCKET clientSocket) {
    UErrorCode status = U_ZERO_ERROR;
    sqlite3* DB;
    sqlite3_stmt* statement;
...
void WebServer::sendBinaryFile(char* url_c_str, SOCKET clientSocket, const std::string &content_type) {
...
bool WebServer::recordLemma(std::string _POST[8], SOCKET clientSocket) {
    ...
    if (!sqlite3_open(m_DB_path, &DB)) {
        ...
        std::string sql_text_str = "";
        if (lemma_meaning == "") {
            sql_text_str = "UPDATE lemmas SET eng_trans" + _POST[3] + " = NULL WHERE lemma_id = ?";
            sql_text = sql_text_str.c_str();
            prep_code = sqlite3_prepare_v2(DB, sql_text, -1, &statement, NULL);
...
bool WebServer::pullInLemma(std::string _POST[4], SOCKET clientSocket) {
...
bool WebServer::retrieveMeanings(std::string _POST[2], SOCKET clientSocket) {
...
bool WebServer::retrieveMultiwordMeanings(std::string _POST[2], SOCKET clientSocket) {
    ...
    if(!sqlite3_open(m_DB_path, &DB)) {
        sqlite3_stmt* statement;
        int prep_code, run_code;
        std::string sql_text_str = "SELECT eng_trans"+_POST[1]+" FROM multiword_lemmas WHERE multiword_id = "+_POST[0];
        prep_code = sqlite3_prepare_v2(DB, sql_text_str.c_str(), -1, &statement, NULL);
        run_code = sqlite3_step(statement);
...
bool WebServer::deleteLemma(std::string _POST[3], SOCKET clientSocket) {
...
bool WebServer::clearTable(SOCKET clientSocket) {
...
bool WebServer::retrieveMultiword(std::string _POST[3], SOCKET clientSocket) {
    ...
    if(!sqlite3_open(m_DB_path, &DB)) {
        ...
        for(int i = -1; i > -51; i--) {
            ...
            if(display_text_word_eng_id) {
                ...
                if(text_word_rawsql != nullptr) text_word = text_word.fromUTF8((const char*)text_word_rawsql);
...
bool WebServer::recordMultiword(std::string _POST[8], SOCKET clientSocket) {
    ...
    if(!sqlite3_open(m_DB_path, &DB)) {
        ...
        std::string sql_text_str = "UPDATE multiword_lemmas SET eng_trans"+_POST[4]+" = ? WHERE multiword_id = ?";
        std::cout << sql_text_str << std::endl;
        sqlite3_prepare_v2(DB, sql_text_str.c_str(), -1, &statement, NULL);
        std::cout << "MW lemma meaning: " << multiword_lemma_meaning << std::endl;
...
bool WebServer::updateMultiwordTranslations(std::string _POST[3], SOCKET clientSocket) {
    ...
    if(!sqlite3_open(m_DB_path, &DB)) {
        ...
        std::string sql_text_str = "UPDATE multiword_lemmas SET eng_trans"+_POST[1]+" = ? WHERE multiword_id = ?";
        sqlite3_prepare_v2(DB, sql_text_str.c_str(), -1, &statement, NULL);
...
bool WebServer::deleteMultiword(std::string _POST[4], SOCKET clientSocket) {
    sqlite3* DB;
...
bool WebServer::addTextOldEnglish(std::string _POST[3], SOCKET clientSocket) {
    std::cout << "update_db.php" << std::endl;
