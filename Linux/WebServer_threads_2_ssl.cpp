...
#include <sstream>
#include "WebServer_threads_ssl.h"
#include <math.h>
#include <sys/stat.h>
#include <vector>

void WebServer::onMessageReceived(ClientConnection* ssl_conn_ptr, const char *msg, int length, int get_or_post, int headers_length, int content_length) {
    ...
    if (get_or_post == GET)
    {
        ...
        if(!strcmp(fil_ext, ".css")) {
            ...
            sendBinaryFile(url_c_str, ssl_conn_ptr, content_type);
            return;
        }
    ...
    //    oss << "Cache-Control: no-cache, private \r\n";
        ...
        sendToClient(ssl_conn_ptr, output.c_str(), size);
    }
        ...
        char url[50];
        getURL(msg, url);
        handlePOSTedData(post_data_c_str, ssl_conn_ptr, (const char*)url);
...
void WebServer::handlePOSTedData(const char* post_data, ClientConnection* ssl_conn_ptr, const char* url) {
    ...
    if(post_fields == -1) {
        ...
        sendToClient(ssl_conn_ptr, bad_post_response.c_str(), bad_post_response.length() + 1);
        return;
    }
    ...
    if(post_fields == 0 && !strcmp(url, "/clear_table.php")) { //clearing the table could be ruinous so adding the extra condition is prudent and should never be run except if post_fields == 0
        ...
        bool php_func_success = clearTable(ssl_conn_ptr);
        return;
    }
    ...
    if(!strcmp(url, "/get_lang_id.php")) {
        bool php_func_success = getLangId(post_values, ssl_conn_ptr);
    }
    else if(!strcmp(url, "/retrieve_engword.php")) {
        bool php_func_success = retrieveEngword(post_values, ssl_conn_ptr);
    }
    else if(!strcmp(url, "/retrieve_meanings.php")) {
        bool php_func_success = retrieveMeanings(post_values, ssl_conn_ptr);
    }
    else if(!strcmp(url, "/lemma_tooltip.php")) {
        bool php_func_success = lemmaTooltips(post_values, ssl_conn_ptr);
    }
    else if(!strcmp(url, "/lemma_tooltip_mw.php")) {
        bool php_func_success = lemmaTooltipsMW(post_values, ssl_conn_ptr);
    }
    else if(!strcmp(url, "/lemma_delete.php")) {
        bool php_func_success = deleteLemma(post_values, ssl_conn_ptr);
    }
    else if(!strcmp(url, "/lemma_record.php")) {
        bool php_func_success = recordLemma(post_values, ssl_conn_ptr);
    }
    else if(!strcmp(url, "/retrieve_text.php")) {
        bool php_func_success = retrieveText(post_values, ssl_conn_ptr);
    }
    else if(!strcmp(url, "/retrieve_text_splitup.php")) {
        bool php_func_success = retrieveTextSplitup(post_values, ssl_conn_ptr);
    }
    else if(!strcmp(url, "/delete_text.php")) {
        bool php_func_success = deleteText(post_values, ssl_conn_ptr);
    }
    else if(!strcmp(url, "/update_db.php")) {
        bool php_func_success = addText(post_values, ssl_conn_ptr);  
    }
    else if(!strcmp(url, "/pull_lemma.php")) {
        bool php_func_success = pullInLemma(post_values, ssl_conn_ptr);
    }
    else if(!strcmp(url, "/retrieve_multiword.php")) {
        bool php_func_success = retrieveMultiword(post_values, ssl_conn_ptr);
    }
    else if(!strcmp(url, "/retrieve_MW_meanings.php")) {
        bool php_func_success = retrieveMultiwordMeanings(post_values, ssl_conn_ptr);
    }
    else if(!strcmp(url, "/record_multiword.php")) {
        bool php_func_success = recordMultiword(post_values, ssl_conn_ptr);
    }
    else if(!strcmp(url, "/delete_multiword.php")) {
        bool php_func_success = deleteMultiword(post_values, ssl_conn_ptr);
    }
    else if(!strcmp(url, "/pull_multiword.php")) {
        bool php_func_success = pullInMultiword(post_values, ssl_conn_ptr);
    }
    else if(!strcmp(url, "/pull_mw_by_form.php")) {
        bool php_func_success = pullInMultiwordByForm(post_values, ssl_conn_ptr);
    }
    else if(!strcmp(url, "/update_MW_translations.php")) {
        bool php_func_success = updateMultiwordTranslations(post_values, ssl_conn_ptr);
    }
    else if(!strcmp(url, "/curl_lookup.php")) {
        bool php_func_success = curlLookup(post_values, ssl_conn_ptr);
    }
    else if(!strcmp(url, "/disregard_word.php")) {
        bool php_func_success = disregardWord(post_values, ssl_conn_ptr);
    }
    else if(!strcmp(url, "/dump_lemmas.php")) {
        bool php_func_success = dumpLemmaTable(post_values, ssl_conn_ptr);
    }
    else if(!strcmp(url, "/add_text_OE.php")) {
        bool php_func_success = addTextOldEnglish(post_values, ssl_conn_ptr);
    }
    else if(!strcmp(url, "/update_displayWord.php")) {
        bool php_func_success = updateDisplayWord(post_values, ssl_conn_ptr);
    }
...
void WebServer::getURL(const char* msg, char* url) {
    int url_start = c_strFind(msg, "/");
  //  printf("url_start: %i\n", url_start);
    int url_end = c_strFind(msg + url_start, " ") + url_start;
  //  printf("url_end: %i\n", url_end);
...
bool WebServer::addText(std::string _POST[3], ClientConnection* ssl_conn_ptr) {
    ...
    if(!sqlite3_open(m_DB_path, &DB)) {
       ...
       /* unicode_text_title.findAndReplace("’", "\'"); */
        ...
        sendToClient(ssl_conn_ptr, POST_response.c_str(), size);
        return true;
    }
    else {
...
bool WebServer::deleteText(std::string _POST[1], ClientConnection* ssl_conn_ptr) {
...
bool WebServer::lemmaTooltips(std::string _POST[2], ClientConnection* ssl_conn_ptr) {
    sqlite3* DB;    
    ...
    if(!sqlite3_open(m_DB_path, &DB)) {
        ...
        sendToClient(ssl_conn_ptr, post_response.str().c_str(), length);
...
bool WebServer::retrieveText(std::string text_id[1], ClientConnection* ssl_conn_ptr) {
    ...
    if(text_id_int == 0) {
        ...
        sendToClient(ssl_conn_ptr, post_response_ss.str().c_str(), length);
...
bool WebServer::retrieveTextSplitup(std::string _POST[3], ClientConnection* ssl_conn_ptr) {
    UErrorCode status = U_ZERO_ERROR;
    sqlite3 *DB;
    sqlite3_stmt *statement;
...
bool WebServer::getLangId(std::string text_id[1], ClientConnection* ssl_conn_ptr) {
    ...
    if(!sqlite3_open(m_DB_path, &DB)) {
        ...
        sendToClient(ssl_conn_ptr, html.str().c_str(), length);
...
void WebServer::sendBinaryFile(char* url_c_str, ClientConnection* ssl_conn_ptr, const std::string &content_type) {
    ...
    if (urlFile.good())
    {
        ...
        if(font_filesize < 1048576) {
            ...
            sendToClient(ssl_conn_ptr, content_buf, headers_size + font_filesize);
    ...
    {
        std::cout << "This file was not opened successfully." << std::endl;
        sendToClient(ssl_conn_ptr, "HTTP/1.1 404 Not Found\r\nContent-Length: 0\r\n\r\n", 46);
        return;
    }
    
}

bool WebServer::retrieveEngword(std::string _POST[3], ClientConnection* ssl_conn_ptr) {
...
bool WebServer::recordLemma(std::string _POST[8], ClientConnection* ssl_conn_ptr) {
...
bool WebServer::pullInLemma(std::string _POST[4], ClientConnection* ssl_conn_ptr) {
    sqlite3* DB;
    sqlite3_stmt* statement;
...
bool WebServer::retrieveMeanings(std::string _POST[2], ClientConnection* ssl_conn_ptr) {
...
bool WebServer::retrieveMultiwordMeanings(std::string _POST[2], ClientConnection* ssl_conn_ptr) {
...
bool WebServer::deleteLemma(std::string _POST[3], ClientConnection* ssl_conn_ptr) {
    ...
    if(!sqlite3_open(m_DB_path, &DB)) {
        ...
        sendToClient(ssl_conn_ptr, POST_response.str().c_str(), 65);
...
bool WebServer::clearTable(ClientConnection* ssl_conn_ptr) {
...
bool WebServer::retrieveMultiword(std::string _POST[3], ClientConnection* ssl_conn_ptr) {
...
bool WebServer::pullInMultiword(std::string _POST[2], ClientConnection* ssl_conn_ptr) {
...
bool WebServer::pullInMultiwordByForm(std::string _POST[4], ClientConnection* ssl_conn_ptr) {
    sqlite3* DB;
    if(!sqlite3_open(m_DB_path, &DB)) {
        sqlite3_stmt* statement1;
...
bool WebServer::recordMultiwordOld(std::string _POST[8], ClientConnection* ssl_conn_ptr) {
    sqlite3* DB;
...
bool WebServer::recordMultiword(std::string _POST[8], ClientConnection* ssl_conn_ptr) {
    sqlite3* DB;
...
bool WebServer::updateMultiwordTranslations(std::string _POST[3], ClientConnection* ssl_conn_ptr) {
    ...
    if(!sqlite3_open(m_DB_path, &DB)) {
        ...
        sendToClient(ssl_conn_ptr, post_response_str.c_str(), length);
...
bool WebServer::deleteMultiword(std::string _POST[4], ClientConnection* ssl_conn_ptr) {
    sqlite3* DB;
    ...
    if(!sqlite3_open(m_DB_path, &DB)) {
        ...
        sendToClient(ssl_conn_ptr, POST_response, 64); //size manually counted, could've messed up
...
bool WebServer::curlLookup(std::string _POST[1], ClientConnection* ssl_conn_ptr) {
...
bool WebServer::disregardWord(std::string _POST[2], ClientConnection* ssl_conn_ptr) {
    sqlite3* DB;
    ...
    if(!sqlite3_open(m_DB_path, &DB)) {
        ...
        sendToClient(ssl_conn_ptr, post_response.c_str(), length); //not commiting the transaction until after the response is sent makes it seem much faster on the frontend, but it is cheeky and probably dangerous
...
bool WebServer::dumpLemmaTable(std::string _POST[1], ClientConnection* ssl_conn_ptr) {
    sqlite3* DB;
...
bool WebServer::addTextOldEnglish(std::string _POST[3], ClientConnection* ssl_conn_ptr) {
    std::cout << "update_db.php" << std::endl;
...
bool WebServer::updateDisplayWord(std::string _POST[5], ClientConnection* ssl_conn_ptr) {
...
bool WebServer::lemmaTooltipsMW(std::string _POST[3], ClientConnection* ssl_conn_ptr) {
    sqlite3* DB;    
